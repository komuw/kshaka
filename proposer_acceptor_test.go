...
func TestPropose(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "two acceptors",
			pa:      &Node{ID: 1, ballot: ballot{}, nodes: []*Node{NewNode(acceptorStore), NewNode(acceptorStore)}},
			args:    args{key: []byte("foo"), changeFunc: readFunc},
			want:    nil,
			wantErr: true,
		},
		{name: "enough acceptors readFunc no key set",
			pa: &Node{ID: 1,
				ballot: ballot{},
				nodes: []*Node{NewNode(acceptorStore),
					NewNode(acceptorStore),
					NewNode(acceptorStore),
					NewNode(acceptorStore)}},
			args:    args{key: []byte("foo"), changeFunc: readFunc},
			want:    nil,
		...
		{name: "enough acceptors readFunc with key set",
			pa: &Node{ID: 1,
				ballot: ballot{},
				nodes: []*Node{NewNode(acceptorStore2),
					NewNode(acceptorStore2),
					NewNode(acceptorStore2),
					NewNode(acceptorStore2)}},
