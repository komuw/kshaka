...
func (a *acceptor) prepare(b ballot, key []byte) (acceptorState, bool, error) {
	...
	if err != nil {
		// TODO: propagate the underlying error
		return acceptorState{state: state, acceptedBallot: acceptedBallot}, false, prepareError(fmt.Sprintf("unable to get promisedBallot of acceptor:%v", a.id))
	}
	...
	if !bytes.Equal(promisedBallotBytes, nil) {
		...
		if promisedBallot.Counter > b.Counter {
			return acceptorState{promisedBallot: promisedBallot, acceptedBallot: acceptedBallot, state: state}, false, prepareError(fmt.Sprintf("submitted ballot:%v is less than ballot:%v of acceptor:%v", b, promisedBallot, a.id))
		}
	}
	...
	err = enc.Encode(b)
	if err != nil {
		return acceptorState{acceptedBallot: acceptedBallot, state: state, promisedBallot: promisedBallot}, false, prepareError(fmt.Sprintf("%v", err))
	}
	...
	return acceptorState{acceptedBallot: acceptedBallot, state: state, promisedBallot: b}, true, nil
}
...
func (a *acceptor) accept(b ballot, key []byte, value []byte) (acceptorState, bool, error) {
	...
	if err != nil {
		// TODO: propagate the underlying error
		return acceptorState{state: state, acceptedBallot: acceptedBallot}, false, acceptError(fmt.Sprintf("unable to get promisedBallot of acceptor:%v", a.id))
	}
	var promisedBallot ballot
	if !bytes.Equal(promisedBallotBytes, nil) {
		// ie we found an promised ballot
		...
		// TODO: also take into account the node ID to resolve tie-breaks
		if promisedBallot.Counter > b.Counter {
			return acceptorState{promisedBallot: promisedBallot, acceptedBallot: acceptedBallot, state: state}, false, acceptError(fmt.Sprintf("submitted ballot:%v is less than ballot:%v of acceptor:%v", b, promisedBallot, a.id))
		}
	}
	...
	if err != nil {
		return acceptorState{acceptedBallot: acceptedBallot, state: state, promisedBallot: promisedBallot}, false, acceptError(fmt.Sprintf("%v", err))
	}
